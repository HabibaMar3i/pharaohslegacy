...
const validationSchema = Yup.object({
    ...
    insurance_no: Yup.string().required("Insurance number is required"),
    role: Yup.string().required("Role is required"),
    experience: Yup.string().required("Experience is required"),
    location: Yup.string().required("Location is required")
});

function ManageTourPersonnel() {
    ...
    useEffect(() => {
        axios.get('http://localhost/admin_tourguide.php')
            .then(response => {
            ...
            .catch(error => {
                setMsg('Failed to load TourPersonnel');
                setLoading(false);
            });
    }, []);
    ...
    async function deletePerson(person_id) {
        try {
            const response = await axios.delete(`http://localhost/admin_tourguide.php?person_id=${person_id}`);
            if (response.data.success) {
                toast.success('Tour personnel deleted successfully', {
                    duration: 1000,
                    position: 'top-center'
                });
            ...
            setMsg('Failed to delete TourPersonnel');
        }
    }

    async function addTourPersonnel(values, { setSubmitting }) {
        try {
            let formData = new FormData();
            Object.keys(values).forEach(key => formData.append(key, values[key]));

            const response = await axios.post('http://localhost/admin_tourguide.php', formData, {
                headers: {
            ...
            if (response.data.success) {
                axios.get('http://localhost/admin_tourguide.php')
                    .then(response => {
                        if (response.data.success) {
                            setPersons(response.data.data);
                            toast.success('Tour personnel added successfully', {
                                duration: 1000,
                                position: 'top-center'
                            });
                        }
                    })
                    .catch(error => setMsg('Failed to load TourPersonnel'));
                formik.resetForm();
                setOpen(false);
            } else {
    ...
    const formik = useFormik({
        initialValues: {
            firstname: '', lastname: '', email: '', password: '', confirmpassword: '', phone: '', DOB: '', gender: '', language: '', ssn: '', insurance_no: '', role: '', experience: '', location: ''
        },
        validationSchema,
        onSubmit: addTourPersonnel
    });
        ...
        <Container>
            <Box sx={{ marginTop: '2rem' }}>
                ...
                <StyledDialog open={open} onClose={handleClose}>
                    ...
                    <DialogContent>
                        <DialogContentText>
                            Please fill out the form below to add a new tour personnel.
                        </DialogContentText>
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12} sm={6}>
                                    <TextField
                                        ...
                                        value={formik.values.insurance_no}
                                        onChange={formik.handleChange}
                                        error={formik.touched.insurance_no && Boolean(formik.errors.insurance_no)}
                                        helperText={formik.touched.insurance_no && formik.errors.insurance_no}
                                        InputProps={{
                                            sx: { borderRadius: '16px' },
                                        }}
                                    />
                ...
                <StyledDialog open={detailOpen} onClose={handleDetailClose}>
                    ...
                    <DialogContent>
                        {selectedPerson && (
                            <DetailBox>
                                <Typography><b>Name:</b> {selectedPerson.person_name}</Typography>
                                ...
                                <Typography><b>Insurance No:</b> {selectedPerson.insurance_no}</Typography>
                                <Typography><b>Role:</b> {selectedPerson.role}</Typography>
                                <Typography><b>Experience:</b> {selectedPerson.experience}</Typography>
                                <Typography><b>Location:</b> {selectedPerson.location}</Typography>
                            </DetailBox>
                        )}
                    </DialogContent>
                    <DialogActions>
...
export default ManageTourPersonnel;
